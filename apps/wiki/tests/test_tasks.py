...
class RebuildTestCase(TestCase):
    ...
    def tearDown(self):
        self.cache.delete(settings.WIKI_REBUILD_TOKEN)
    ...
    def test_eager_queue(self, delay):
        schedule_rebuild_kb()
        assert not self.cache.get(settings.WIKI_REBUILD_TOKEN)
    ...
    def test_task_queue(self, delay):
        ...
        schedule_rebuild_kb()
        assert self.cache.get(settings.WIKI_REBUILD_TOKEN)
        assert delay.called

    @override_settings(WIKI_REBUILD_ON_DEMAND=True)
    @mock.patch_object(rebuild_kb, 'delay')
    def test_already_queued(self, delay):
        self.cache.set(settings.WIKI_REBUILD_TOKEN, True)
        schedule_rebuild_kb()
        assert self.cache.get(settings.WIKI_REBUILD_TOKEN)
        assert not delay.called
    ...
    @override_settings(WIKI_REBUILD_ON_DEMAND=True)
    @mock.patch_object(rebuild_kb, 'delay')
    def test_dont_queue(self, delay):
        with mock.patch_object(self.cache, 'get') as get:
