...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        UPDATE_SQL = """UPDATE wiki_revision
        SET toc_depth=%d
        WHERE show_toc=%d;
        """
        showing_toc = UPDATE_SQL % (1, 1)
        not_showing_toc = UPDATE_SQL % (0, 0)
        db.execute(UPDATE_SQL % (1, 1))
        db.execute(UPDATE_SQL % (0, 0))

    def backwards(self, orm):
        "Write your backwards methods here."
        db.execute("""UPDATE wiki_revision
        SET show_toc=1
        WHERE toc_depth>=1;
        """)
        db.execute("""UPDATE wiki_revision
