...
def test_gather_no_resources():
    ...
    requester = mock_requester(
        response_spec=["status_code", "json"], status_code=200, json=meta_without_trans
    )
    storage = mock_storage(spec=["get_document_metadata", "save_document_metadata"])
...
def test_extract_translation_with_depth():
    ...
    result_opts = {"depth": 1}
    assert result == [
        ("document", "/fr/docs/Apprendre/Commencer_avec_le_web", result_opts),
        ("document", "/tr/docs/Öğren/Getting_started_with_the_web", result_opts),
    ]
