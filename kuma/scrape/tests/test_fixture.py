...
def test_load_constance():
    ...
    spec = {"database.constance": [{"key": "KUMASCRIPT_MAX_AGE", "value": 321}]}
...
def test_load_group():
    ...
    spec = {
        "auth.group": [
            {"name": "Attachment Moderators", "permissions": [["change_attachment"]]}
        ],
        "auth.permission": [
            {
...
def test_missing_relation_is_error():
    ...
    spec = {"users.user": [{"username": "captain_america", "groups": [["SHIELD"]]}]}
...
def test_user_password():
    ...
    spec = {"users.user": [{"username": "wagstaff", "password": "swordfish"}]}
    FixtureLoader(spec).load()
