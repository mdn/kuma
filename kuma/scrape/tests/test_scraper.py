...
def test_request():
    ...
    mock_session.get.assert_called_once_with("https://example.com/path", timeout=1.0)
...
def test_request_no_raise():
    ...
    assert response is mock_response
    mock_session.get.assert_called_once_with("http://example.net/path", timeout=1.0)
...
@pytest.mark.parametrize(
    "retry_after,sleep_time", rate_limit_tests.values(), ids=list(rate_limit_tests)
)
    ...
    def gather(self, requester, storage):
        ...
        if self.depth:
            sources.append(
                (
                    "fake",
                    "%s%d" % (self.name, self.depth),
                    {"length": self.length, "depth": self.depth - 1},
                )
            )
