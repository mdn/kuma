...
class Command(ScrapeCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "spec", metavar="specification.json", help="Sample specification file"
        )
        parser.add_argument(
            "--host",
            help=("Where to sample MDN from (default" ' "wiki.developer.mozilla.org")'),
        ...
        parser.add_argument(
            "--force", help="Update existing records", action="store_true"
        )
    ...
    def handle(self, *arg, **options):
        ...
        if incomplete:
            raise CommandError(
                "%d source%s incomplete." % (incomplete, "" if incomplete == 1 else "s")
            )
