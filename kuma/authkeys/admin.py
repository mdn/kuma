...
def history_link(self):
    url = f'{reverse("admin:authkeys_keyaction_changelist")}' f"?key__exact={self.id}"
...
class KeyAdmin(admin.ModelAdmin):
    fields = ("description",)
    list_display = ("id", "user", "created", history_link, "key", "description")
...
def key_link(self):
    key = self.key
    url = reverse("admin:authkeys_key_change", args=[key.id])
