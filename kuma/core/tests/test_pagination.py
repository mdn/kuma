...
def test_paginated_url():
    ...
    assert paginated.url == request.build_absolute_uri(request.path) + "?q=bookmarks"
...
def test_invalid_page_param():
    ...
    queryset = range(100)
    paginated = paginate(request, queryset)
    assert paginated.url == request.build_absolute_uri(request.path) + "?"
...
def test_paginator_filter_current_selected():
    ...
    html = paginator(pager)
    doc = pyquery.PyQuery(html)
    assert doc("li.selected a").attr("href") == "http://testserver/en-US/search?page=10"
