...
def dashboard_revisions(wiki_user, wiki_user_2, wiki_user_3):
    ...
    for user, lang in zip(users, sorted(languages.keys())):
        ...
        trans_title, trans_content = (fmt % user.username for fmt in languages[lang])
...
def test_revisions_show_spam_submission_button(
    has_perm, root_doc, wiki_user, user_client
):
    """Submit as spam button appears when the user has the permission."""
    if has_perm:
        content_type = ContentType.objects.get_for_model(RevisionAkismetSubmission)
        perm = Permission.objects.get(
            codename="add_revisionakismetsubmission", content_type=content_type
        )
...
def test_revisions_locale_filter(dashboard_revisions, user_client):
    ...
    response = user_client.get(
        url, HTTP_HOST=settings.WIKI_HOST, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
    )
    assert response.status_code == 200
...
@pytest.mark.parametrize(
    "authors",
    (
        RevisionDashboardForm.KNOWN_AUTHORS,
        RevisionDashboardForm.UNKNOWN_AUTHORS,
        RevisionDashboardForm.ALL_AUTHORS,
    ),
)
def test_revisions_known_authors_filter(
    authors, dashboard_revisions, user_client, known_author
):
...
def test_revisions_deleted_document(dashboard_revisions, user_client, wiki_user):
...
class SpamDashTest(SampleRevisionsMixin, UserTestCase):
    ...
    def test_not_logged_in(self, mock_analytics_upageviews):
        ...
        response = self.client.get(
            reverse("dashboards.spam"), HTTP_HOST=settings.WIKI_HOST
        )
        assert response.status_code == 302
        assert_no_cache_header(response)
    ...
    def test_permissions(self, mock_analytics_upageviews):
        ...
        assert_no_cache_header(response)
        assert "text/html" in response["Content-Type"]
        assert "dashboards/spam.html" in (
            template.name for template in response.templates
        )

    def test_misconfigured_google_analytics_does_not_block(
        self, mock_analytics_upageviews
    ):
        ...
        rev = self.create_revisions(
            num=1, creator=self.testuser, document=self.document
        )[0]
        ...
        self.assertContains(response, "The report is being processed", status_code=200)
    ...
    def test_recent_spam_revisions_show(self, mock_analytics_upageviews):
        ...
        for revision in created_revisions:
            document_url = reverse(
                "wiki.document", kwargs={"document_path": revision.document.slug}
            )
    ...
    def test_spam_trends_stats(self, mock_analytics_upageviews):
        ...
        revs = self.create_revisions(
            num=15, creator=self.testuser, document=self.document
        )
        for i in range(0, 3):
        ...
        row_daily = (
            page.find(".spam-trends-table tbody tr")[0]
            .text_content()
            .replace(" ", "")
            .strip("\n")
            .split("\n")
        ...
        row_weekly = (
            page.find(".spam-trends-table tbody tr")[1]
            .text_content()
            .replace(" ", "")
            .strip("\n")
            .split("\n")
        ...
        row_monthly = (
            page.find(".spam-trends-table tbody tr")[2]
            .text_content()
            .replace(" ", "")
            .strip("\n")
            .split("\n")
        ...
        row_quarterly = (
            page.find(".spam-trends-table tbody tr")[3]
            .text_content()
            .replace(" ", "")
            .strip("\n")
            .split("\n")
...
def test_index(client, db):
    ...
    response = client.get(reverse("dashboards.index"), HTTP_HOST=settings.WIKI_HOST)
    assert response.status_code == 200
    content = response.content.decode(response.charset)
    assert reverse("dashboards.macros") in content
    assert reverse("dashboards.spam") not in content
    l10n_url = reverse("wiki.list_with_localization_tag", kwargs={"tag": "inprogress"})
...
def test_index_non_english_sees_translations(client, db):
    ...
    with override("fr"):
        ...
        assert reverse("dashboards.spam") not in content
        l10n_url = reverse(
            "wiki.list_with_localization_tag", kwargs={"tag": "inprogress"}
        )
...
def test_index_admin_sees_spam_dashboard(admin_client):
    ...
    response = admin_client.get(
        reverse("dashboards.index"), HTTP_HOST=settings.WIKI_HOST
    )
    assert response.status_code == 200
    content = response.content.decode(response.charset)
    assert reverse("dashboards.macros") in content
