...
def get_mocked_s3_bucket():
    def get_s3_response(**kwargs):
        ...
        if len(deleted) > 1:
            ...
            error.update(Code="InternalError", Message="Some error")
            return {"Deleted": deleted[1:], "Errors": [error]}
...
def test_publish_standard(get_s3_bucket_mock, root_doc, invalidate_cdn_cache):
    ...
    with mock.patch("kuma.api.tasks.request_cdn_cache_invalidation") as mocked:
        ...
        if invalidate_cdn_cache:
            mocked.delay.assert_called_once_with([(root_doc.locale, root_doc.slug)])
        else:
...
def test_publish_redirect(get_s3_bucket_mock, root_doc, redirect_doc):
    ...
    s3_bucket_mock.put_object.assert_called_once_with(
        ...
        Body=json.dumps(
            document_api_data(
                None, redirect_url=get_content_based_redirect(redirect_doc)[0]
            )
        ),
    )
    log_mock.info.assert_called_once_with("Published S3 Object #1")
...
def test_publish_redirect_to_other(get_s3_bucket_mock, redirect_to_macros):
    ...
    s3_bucket_mock.put_object.assert_called_once_with(
        ...
        Body=json.dumps(
            document_api_data(
                redirect_url=absolutify("/en-US/dashboards/macros", for_wiki_site=True)
            )
        ),
        ContentType="application/json",
...
def test_publish_multiple(
    get_s3_bucket_mock, root_doc, redirect_doc, redirect_to_home, trans_doc
):
    """
...
@mock.patch("kuma.api.tasks.get_s3_bucket")
def test_unpublish_multiple(
    get_s3_bucket_mock, root_doc, redirect_doc, redirect_to_home
):
    """
...
def test_unpublish_multiple_chunked(
    get_s3_bucket_mock, root_doc, redirect_doc, redirect_to_home
):
    """
    ...
    s3_bucket_mock.delete_objects.assert_has_calls(
        [
            mock.call(Delete={"Objects": [{"Key": key} for key in s3_keys[:2]]}),
            mock.call(Delete={"Objects": [{"Key": key} for key in s3_keys[2:]]}),
        ]
    )
    log_mock.error.assert_has_calls(
        [
            mock.call("Unable to unpublish {}: (InternalError) Some error".format(key))
            for key in s3_keys[:2]
        ]
    )
    log_mock.info.assert_has_calls(
        [mock.call("Unpublished {}".format(s3_keys[-1])), mock.call("Done!")]
    )


def test_request_cdn_cache_invalidation_not_configured(
    settings, mocked_get_cloudfront_client
):
...
def test_request_cdn_cache_invalidation_configured(
    ...
    settings.MDN_CLOUDFRONT_DISTRIBUTIONS = {
        "mything": {
            "id": "XYZABC123",
            "transform_function": ("kuma.api.tests.test_tasks.transformer"),
        },
        "unconfigured": {"id": None, "transform_function": "wont.be.used"},
    }
    ...
    mocked_get_cloudfront_client().create_invalidation.assert_called_with(
        ...
        InvalidationBatch={
            "Paths": {"Items": ["/sv-SE/Learn/stuff/"], "Quantity": 1},
