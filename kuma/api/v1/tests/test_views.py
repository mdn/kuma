...
def test_bc_signal_happy_path(client, root_doc):
    with override_flag('bc-signals', True):
...
@pytest.mark.django_db
def test_bc_signal_waffle_flag(client):
    with override_flag('bc-signals', False):
        url = reverse('api.v1.bc_signal')
        response = client.post(url)
        assert response.status_code == 404
...
@pytest.mark.django_db
def test_bc_signal_explanation_validation(client, root_doc):
    with override_flag('bc-signals', True):
        url = reverse('api.v1.bc_signal')
        data = {
            'slug': root_doc.slug,
        ...
        data['explanation'] = 'tooshort'
        response = client.post(url, data)
        assert response.json()['explanation']

        data['explanation'] = 'x' * 1001
        response = client.post(url, data)
        assert response.status_code == 400
        assert response.json()['explanation']
