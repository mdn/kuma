    ...
    for change in changes:
        if change["event"] == 'added_stable':
            for feature in change['features']:
                publish_notification(feature['path'],
                                     f"is now stable in {change['browser']} {change['version']}",
                                     dry_run=dry_run, data=change)
            ...
            publish_notification(change['path'], f'Support information added for {text}',
                                 dry_run=dry_run, data=change)
        elif change["event"] == 'added_subfeatures':
            publish_notification(change["path"], 'has new subfeatures',
                                 dry_run=dry_run, data=change)
