...
from kuma.wiki.models import Document, DocumentDeletionLog, RevisionAkismetSubmission


class Command(BaseCommand):
    help = (
        "Seed Akismet spam with documents that were deleted " "because they were spam"
    )
    ...
    def handle(self, *args, **options):
        ...
        for i, logged_deletion in enumerate(logged_deletions.iterator(), 1):
            ...
            if dry_run:
                ...
                self.stdout.write(
                    "Not submitting current revision %s of "
            ...
            try:
                ...
                self.stderr.write(
                    "Akismet error while submitting current "
                    "revision %s of document %s: %s"
                    % (document.current_revision.pk, document.pk, exc.debug_help)
                )
            else:
                self.stdout.write(
                    "Successfully submitted current "
