...
def test_section_pars_for_empty_docs(db):
    doc = document(title="Doc", locale="fr", slug="doc", save=True, html="<!-- -->")
...
def test_section_ids():
    ...
    result_src = kuma.wiki.content.parse(doc_src).injectSectionIDs().serialize()
...
def test_incremented_section_ids():
    ...
    result_src = kuma.wiki.content.parse(expected).injectSectionIDs().serialize()
...
def test_extractSection_ignore_heading():
    ...
    result = parse(doc_src).extractSection(id="s4-1", ignore_heading=True).serialize()
    assert normalize_html(result) == normalize_html(expected)
...
def test_basic_section_replace():
    ...
    """
    result = (
        kuma.wiki.content.parse(doc_src)
...
def test_ignore_heading_section_replace():
    ...
    result = (
        kuma.wiki.content.parse(doc_src)
        .replaceSection(id="s2", replace_src=replace_src, ignore_heading=True)
        .serialize()
    )
    assert normalize_html(expected) == normalize_html(result)
...
def test_basic_section_remove():
    ...
    expected = """
        ...
        <div class="here">Leave <span>this</span>.</div>
    """
    result = kuma.wiki.content.parse(doc_src).removeSection("here").serialize()
    assert normalize_html(expected) == normalize_html(result)
...
def test_code_syntax_conversion():
    ...
    """
    result = kuma.wiki.content.parse(doc_src).filter(CodeSyntaxFilter).serialize()
    assert normalize_html(expected) == normalize_html(result)


def test_non_ascii_section_headers():
    headers = [
        ("Documentation à propos de HTML", "Documentation_à_propos_de_HTML"),
        (
        ...
        ("Lektury uzupełniające", "Lektury_uzupełniające"),
        ("Атрибуты", "Атрибуты"),
        ("HTML5 엘리먼트", "HTML5_엘리먼트"),
        ("Non safe title \"#$%&+,/:;=?@[\\]^`{|}~')(", "Non_safe_title"),
        ("Five o'clock", "Five_oclock"),
    ]
...
def test_generate_toc():
    ...
    """
    result = kuma.wiki.content.parse(doc_src).filter(SectionTOCFilter).serialize()
    assert normalize_html(expected) == normalize_html(result)
...
def test_generate_toc_h2():
    ...
    """
    result = kuma.wiki.content.parse(doc_src).filter(H2TOCFilter).serialize()
    assert normalize_html(expected) == normalize_html(result)
...
def test_generate_toc_h3():
    ...
    """
    result = kuma.wiki.content.parse(doc_src).filter(H3TOCFilter).serialize()
    assert normalize_html(expected) == normalize_html(result)
...
FILTERIFRAME_ACCEPTED = {
    "youtube_ssl": (
        "https://www.youtube.com/embed/" "iaNoBlae5Qw/?feature=player_detailpage"
    ),
    "prod": (
    ...
    "github.io": (
        "https://mdn.github.io/webgl-examples/" "tutorial/sample6/index.html"
    ),
    "ie_moz_net": (
    ...
    "interactive": (settings.INTERACTIVE_EXAMPLES_BASE + "/pages/http/headers.html"),
}
...
FILTERIFRAME_REJECTED = {
    ...
    "vagrant_2": (
        "http://developer-local:81/" "en-US/docs/Test$samples/sample1?revision=123"
    ),
    "cdn": ("https://developer.cdn.mozilla.net/is/this/valid?"),
    "stage": (
        "https://stage-files.mdn.moz.works/" "fr/docs/Test$samples/sample2?revision=234"
    ),
...
@pytest.mark.parametrize(
    "url", list(FILTERIFRAME_ACCEPTED.values()), ids=list(FILTERIFRAME_ACCEPTED)
)
...
@pytest.mark.parametrize(
    "url", list(FILTERIFRAME_REJECTED.values()), ids=list(FILTERIFRAME_REJECTED)
)
...
@pytest.mark.parametrize(
    "href", list(BLEACH_INVALID_HREFS.values()), ids=list(BLEACH_INVALID_HREFS)
)
...
@pytest.mark.parametrize(
    "href", list(BLEACH_VALID_HREFS.values()), ids=list(BLEACH_VALID_HREFS)
)
...
def test_annotate_links_encoded_utf8(db):
    ...
    Document.objects.create(locale="fr", slug="CSS/Héritage", title="Héritée")
    html = normalize_html('<li><a href="/fr/docs/CSS/H%c3%a9ritage">Héritée</a></li>')
...
def test_annotate_links_uilocale_to_nonexisting_doc(db):
    ...
    actual_raw = parse(html).annotateLinks(base_url=AL_BASE_URL).serialize()
    expected = normalize_html('<li><a rel="nofollow" class="new" href="%s"></li>' % url)
...
class FilterEditorSafetyTests(TestCase):
    def test_editor_safety_filter(self):
        ...
        expected_src = """
            ...
            <p>test</p>
        """
        result_src = kuma.wiki.content.parse(doc_src).filterEditorSafety().serialize()
        assert normalize_html(expected_src) == normalize_html(result_src)
...
@pytest.mark.parametrize("tag", ("br", "command", "img", "input",))
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ('<div class="foo" >foo</div>', '<div class="foo">foo</div>'),
        (
            (
...
@pytest.mark.parametrize("method", ("macro_names", "css_classnames", "html_attributes"))
...
def test_extractor_section(root_doc, annotate_links):
    ...
    result = root_doc.extract.section(
        content, "Quick_Links", annotate_links=annotate_links
    )
    assert normalize_html(result) == normalize_html(expected)
...
@pytest.mark.parametrize(
    "wrapper",
    list(SUMMARY_PLUS_SEO_WRAPPERS.values()),
    ids=list(SUMMARY_PLUS_SEO_WRAPPERS),
)
@pytest.mark.parametrize(
    "markup, text", list(SUMMARY_CONTENT.values()), ids=list(SUMMARY_CONTENT)
)
def test_summary_section(markup, text, wrapper):
    ...
    assert normalize_html(
        get_seo_description(content, "en-US", False)
    ) == normalize_html(markup)


@pytest.mark.parametrize(
    "wrapper", list(SUMMARY_WRAPPERS.values()), ids=list(SUMMARY_WRAPPERS)
)
@pytest.mark.parametrize(
    "markup, expected_markup, text",
