...
@pytest.mark.parametrize("endpoint", ["children", "toc", "json", "json_slug"])
...
@pytest.mark.parametrize(
    "section_case", ("no-section", "section", "another-section", "non-existent-section")
)
def test_api_safe(client, section_doc, section_case, if_none_match, method):
    ...
    if (if_none_match == "match") and (method == "GET"):
        ...
        assert response["x-kuma-revision"] == str(section_doc.current_revision_id)
...
def test_api_put_forbidden_when_no_authkey(client, user_client, root_doc, user_case):
    """
...
@pytest.mark.parametrize("content_case", ("multipart", "json", "html-fragment", "html"))
@pytest.mark.parametrize(
    "section_case", ("no-section", "section", "another-section", "non-existent-section")
)
def test_api_put_existing(
    settings, client, section_doc, authkey, section_case, content_case, if_match
):
    """
    ...
    headers = dict(HTTP_AUTHORIZATION=authkey.header, HTTP_HOST=settings.WIKI_HOST)
    ...
    response = client.put(url, data=encoded_data, content_type=content_type, **headers)
    ...
    if if_match == "mismatch":
        ...
        if content_case in ("multipart", "json"):
            ...
            assert set(rev.review_tags.names()) == set(data["review_tags"].split(","))


@pytest.mark.parametrize("slug_case", ("root", "child", "nonexistent-parent"))
@pytest.mark.parametrize("content_case", ("multipart", "json", "html-fragment", "html"))
@pytest.mark.parametrize("section_case", ("no-section", "section"))
def test_api_put_new(
    settings, client, root_doc, authkey, section_case, content_case, slug_case
):
    """
...
def test_apply_content_experiment_selected_variant(ce_settings, rf):
    ...
    mock_get.assert_called_once_with(locale="en-US", slug="Experiment:Test/Variant")
    assert experiment_doc == db_doc
...
def test_document_banned_ip_can_read(client, root_doc):
    ...
    response = client.get(
        root_doc.get_absolute_url(), REMOTE_ADDR=ip, HTTP_HOST=settings.WIKI_HOST
    )
    assert response.status_code == 200


@pytest.mark.parametrize("endpoint", ["document", "preview"])
def test_kumascript_error_reporting(
    admin_client, root_doc, ks_toolbox, endpoint, mock_requests
):
    """
        ...
        if endpoint == "preview":
            ...
            with mock.patch("kuma.wiki.models.config", **ks_settings):
                response = admin_client.get(
                    root_doc.get_absolute_url(), HTTP_HOST=settings.WIKI_HOST
                )
    ...
    create_link = (
        '#kserrors-list a[href="https://github.com/' 'mdn/kumascript#updating-macros"]'
    )
...
def test_tags_show_in_document(root_doc, client, wiki_user):
    ...
    response = client.get(root_doc.get_absolute_url(), HTTP_HOST=settings.WIKI_HOST)
    assert response.status_code == 200
...
@pytest.mark.parametrize("endpoint", ["wiki.subscribe", "wiki.subscribe_to_tree"])
...
def test_watch_unwatch(user_client, wiki_user, root_doc, endpoint, event):
    ...
    assert response.status_code == 302
    assert_no_cache_header(response)
    assert response["Location"].endswith(reverse("wiki.document", args=[root_doc.slug]))
    ...
    assert_no_cache_header(response)
    assert response["Location"].endswith(reverse("wiki.document", args=[root_doc.slug]))
    assert not event.is_notifying(wiki_user, root_doc), "Watch was not destroyed"


def test_deleted_doc_anon(deleted_doc, client):
    """Requesting a deleted doc returns 404"""
    response = client.get(deleted_doc.get_absolute_url(), HTTP_HOST=settings.WIKI_HOST)
    assert response.status_code == 404
    content = response.content.decode(response.charset)
...
def test_deleted_doc_user(deleted_doc, user_client):
    ...
    response = user_client.get(
        deleted_doc.get_absolute_url(), HTTP_HOST=settings.WIKI_HOST
    )
    assert response.status_code == 404
    content = response.content.decode(response.charset)
    assert "This document was deleted" not in content
...
def test_deleted_doc_no_purge_permdeleted(
    deleted_doc, wiki_moderator, moderator_client
):
...
@pytest.mark.parametrize("href", ["//davidwalsh.name", "http://davidwalsh.name"])
@mock.patch("kuma.wiki.kumascript.get")
def test_redirects_only_internal(
    mock_kumascript_get, constance_config, wiki_user, client, href
):
...
@mock.patch("kuma.wiki.kumascript.get")
def test_self_redirect_supression(
    mock_kumascript_get, constance_config, wiki_user, client
):
...
def test_wiki_only_query_params(
    mock_ssr, mock_kumascript_get, constance_config, client, root_doc, param, status
):
    """
