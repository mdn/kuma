...
def test_purge_get(deleted_doc, moderator_client):
    ...
    assert response.status_code == 200
    assert_no_cache_header(response)
    assert "This document was deleted by" in response.content.decode(response.charset)
...
def test_purge_get_no_log(deleted_doc, moderator_client):
    ...
    assert response.status_code == 200
    assert_no_cache_header(response)
    assert "deleted, for unknown reasons" in response.content.decode(response.charset)
...
def test_revert_get(root_doc, moderator_client):
    url = reverse(
        "wiki.revert_document", args=[root_doc.slug, root_doc.current_revision.id]
    )
    response = moderator_client.get(url, HTTP_HOST=settings.WIKI_HOST)
    assert response.status_code == 200
    assert_no_cache_header(response)
...
def test_delete_post(root_doc, moderator_client):
    ...
    response = moderator_client.post(
        url, data=dict(reason="test"), HTTP_HOST=settings.WIKI_HOST
    )
    assert response.status_code == 302
    assert response["Location"].endswith(root_doc.get_absolute_url())
    assert_no_cache_header(response)
    assert (
        len(
            Document.admin_objects.filter(
                slug=root_doc.slug, locale=root_doc.locale, deleted=True
            )
        )
        == 1
    )
    ...
    assert (
        len(
            DocumentDeletionLog.objects.filter(
                locale=root_doc.locale, slug=root_doc.slug, reason="test"
            )
        )
        == 1
    )
...
def test_purge_post(root_doc, moderator_client):
    ...
    response = moderator_client.post(
        url, data=dict(confirm="true"), HTTP_HOST=settings.WIKI_HOST
    )
    assert response.status_code == 302
    assert response["Location"].endswith(root_doc.get_absolute_url())
    assert_no_cache_header(response)
...
def test_revert_post(edit_revision, moderator_client):
    ...
    url = reverse("wiki.revert_document", args=[root_doc.slug, first_revision.id])
    response = moderator_client.post(
        url, data=dict(comment="test"), HTTP_HOST=settings.WIKI_HOST
    )
    assert response.status_code == 302
