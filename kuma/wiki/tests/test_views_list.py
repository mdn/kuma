...
@pytest.mark.parametrize(
    ...
    [
        ...
        "list_review_tag",
        "list_review",
        "list_with_localization_tag",
        "list_with_localization_tags",
    ],
)
...
def test_revisions_of_translated_document(trans_doc, client):
    ...
    url = reverse(
        "wiki.document_revisions", args=(trans_doc.slug,), locale=trans_doc.locale
    )
    resp = client.get(url, HTTP_HOST=settings.WIKI_HOST)
    assert resp.status_code == 200
...
@pytest.mark.parametrize("locale", ["en-US", "de", "fr"])
def test_list_with_localization_tag(redirect_doc, doc_hierarchy, client, locale):
    top_doc = doc_hierarchy.top
    bottom_doc = doc_hierarchy.bottom
    de_doc = top_doc.translated_to("de")
    for doc in (top_doc, bottom_doc, de_doc, redirect_doc):
    ...
    url = reverse(
        "wiki.list_with_localization_tag", locale=locale, kwargs={"tag": "inprogress"}
    )
    resp = client.get(url, HTTP_HOST=settings.WIKI_HOST)
    dom = pq(resp.content)
    assert resp.status_code == 200
...
@pytest.mark.parametrize("locale", ["en-US", "de", "fr"])
def test_list_with_localization_tags(redirect_doc, doc_hierarchy, client, locale):
    top_doc = doc_hierarchy.top
    bottom_doc = doc_hierarchy.bottom
    de_doc = top_doc.translated_to("de")
    for doc in (top_doc, bottom_doc, de_doc, redirect_doc):
