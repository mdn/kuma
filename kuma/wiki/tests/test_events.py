...
def test_notification_context_for_create(create_revision):
    ...
    utm_campaign = (
        "?utm_campaign=Wiki+Doc+Edits&utm_medium=email"
...
def test_edit_document_event_fires_union(mock_fire, create_revision, wiki_user):
...
def test_first_edit_email_on_create(create_revision):
    ...
    assert mail.subject == (
        "[MDN][en-US][New] wiki_user made their first edit," " creating: Root Document"
    )
...
def test_first_edit_email_on_change(edit_revision):
    ...
    assert mail.subject == (
        "[MDN][en-US] wiki_user made their first edit," " to: Root Document"
    )
...
def test_first_edit_email_on_translate(trans_revision):
    ...
    assert mail.subject == (
        "[MDN][fr][New] wiki_user made their first edit,"
...
def test_spam_attempt_email_on_create(wiki_user):
    ...
    assert mail.subject == (
        "[MDN] Wiki spam attempt recorded with title" " My new spam page"
    )
    assert mail.extra_headers == {"X-Kuma-Editor-Username": "wiki_user"}
...
def test_spam_attempt_email_on_change(wiki_user, root_doc):
    ...
    mail = spam_attempt_email(spam_attempt)
    assert mail.subject == (
        "[MDN] Wiki spam attempt recorded for document"
...
def test_spam_attempt_email_on_translate(wiki_user, trans_doc):
    ...
    mail = spam_attempt_email(spam_attempt)
    assert mail.subject == (
        "[MDN] Wiki spam attempt recorded with title" " Ma nouvelle page de spam"
    )
...
def test_spam_attempt_email_partial_model(wiki_user):
    ...
    spam_attempt = DocumentSpamAttempt(
        user=wiki_user, slug="my-new-spam-page", created=datetime(2017, 4, 14, 15, 13)
    )
    mail = spam_attempt_email(spam_attempt)
