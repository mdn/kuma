...
@pytest.mark.parametrize('qs,expected', (
    ...
    ('?mode=remove', ('Please specify one or more comma-separated macro names '
...
def test_revision_api_get_400(doc_with_macros, client, qs, expected):
    ...
    url = reverse('wiki.revision_api', args=[doc_with_macros.slug],
...
@pytest.mark.parametrize('qs,expected', (
    ...
    ids=('no-change', 'remove-single', 'remove-multiple', 'remove-miss',
         'render-miss')
)
def test_revision_api_get(doc_with_macros, client, constance_config, qs,
                          expected):
    """
...
def test_revision_api_post(doc_with_macros, wiki_user_2, wiki_user_2_token,
                           client, case):
    """
    ...
    assert response['Location'].endswith(reverse(
        'wiki.revision',
...
def test_revision_api_conditional_post(doc_with_macros, wiki_user_2,
                                       wiki_user_2_token, constance_config,
                                       client):
    """
    ...
    response = client.get(
        url + '?mode=remove&macros=m1', HTTP_HOST=settings.WIKI_HOST)
    assert response.status_code == 200
