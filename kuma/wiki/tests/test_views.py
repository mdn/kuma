...
class ViewTests(UserTestCase, WikiTestCase):
    ...
    def test_children_view(self):
        ...
        def _make_doc(title, slug, parent=None, is_redir=False):
            ...
            if is_redir:
                ...
                revision(
                    document=doc,
                    content=test_content,
                    summary=get_seo_description(test_content, strip_markup=False),
                    save=True,
                )
            doc.html = content
            if parent:
        ...
        grandchild_doc_2 = _make_doc(
            "Grandchild 2", "Root/Child_1/Grandchild_2", child_doc_1
        )
        _make_doc(
            "Great Grandchild 1",
        ...
        for expand in (True, False):
            ...
            assert json_obj["subpages"][0]["subpages"][1]["title"] == "Grandchild 2"

        # Depth parameter testing
        def _depth_test(depth, aught):
            url = reverse("wiki.children", args=["Root"]) + "?depth=" + str(depth)
            resp = self.client.get(url)
            assert resp.status_code == 200
            assert_shared_cache_header(resp)
            assert resp["Access-Control-Allow-Origin"] == "*"
            json_obj = json.loads(resp.content)
            assert len(json_obj["subpages"][0]["subpages"][1]["subpages"]) == aught
        ...
        assert_shared_cache_header(resp)
        assert resp["Access-Control-Allow-Origin"] == "*"
        assert json.loads(resp.content) == {"error": "Document does not exist."}
    ...
    @mock.patch(
        "kuma.wiki.jobs.DocumentContributorsJob.get",
        return_value=[
    ...
    def test_footer_contributors(self, get_contributors, flag_is_active):
        ...
        resp = self.client.get(
            rev.document.get_absolute_url(), HTTP_HOST=settings.WIKI_HOST
        )
        assert resp.status_code == 200
        assert_shared_cache_header(resp)
        page = pq(resp.content)
        contributors = page.find(":contains('Contributors to this page')").parents(
            ".contributors-sub"
        )
    ...
    def test_revision_view_bleached_content(self):
        ...
        resp = self.client.get(rev.get_absolute_url(), HTTP_HOST=settings.WIKI_HOST)
        page = pq(resp.content)
    ...
    def test_article_revision_content(self):
        ...
        resp = self.client.get(r.get_absolute_url(), HTTP_HOST=settings.WIKI_HOST)
        page = pq(resp.content)
...
class KumascriptIntegrationTests(UserTestCase, WikiTestCase):
    ...
    def test_preview_nonascii(self, mock_requests):
        ...
        resp = self.client.post(
            reverse("wiki.preview"), {"content": content}, HTTP_HOST=settings.WIKI_HOST
        )
        assert_no_cache_header(resp)
...
class DocumentSEOTests(UserTestCase, WikiTestCase):
    ...
    def test_seo_title(self):
        ...
        def _make_doc(title, aught_titles, slug):
            ...
            response = self.client.get(
                reverse("wiki.document", args=[slug]), HTTP_HOST=settings.WIKI_HOST
            )
        ...
        _make_doc(
            "Special Φ Char",
            ["Special \u03a6 Char - One | MDN", "Special \xce\xa6 Char - One | MDN"],
    ...
    def test_seo_script(self):
        ...
        make_page_and_compare_seo(
            "four",
            '<div class="geckoVersion">' "<p>No no no</p></div><p>yes yes yes</p>",
            "yes yes yes",
        )
        ...
        make_page_and_compare_seo("six", "<p>« CSS</p><p>I am me!</p>", "I am me!")
        # Take the seoSummary class'd element
        make_page_and_compare_seo(
            "seven",
            "<p>I could be taken</p>" '<p class="seoSummary">I should be though</p>',
            "I should be though",
        )
...
@pytest.mark.parametrize(
    "content,expected",
    [
        ("<div onclick=\"alert('hacked!')\">click me</div>", "<div>click me</div>"),
        (
...
class DocumentEditingTests(UserTestCase, WikiTestCase):
    ...
    def test_create_on_404(self):
        ...
        response = self.client.get(
            reverse("wiki.document", args=["noExist"]), HTTP_HOST=settings.WIKI_HOST
        )
        ...
        response = self.client.get(
            reverse("wiki.document", args=["Template:NoExist"]),
            HTTP_HOST=settings.WIKI_HOST,
        )
        assert response.status_code == 302
        assert "public" not in response["Cache-Control"]
    ...
    def test_retitling_solo_doc(self):
        ...
        * Doesn't create redirect
        ...
        old_title = doc.title
        data = new_document_data()
        data.update({"title": new_title, "form-type": "rev"})
        data["slug"] = ""
    ...
    def test_retitling_parent_doc(self):
        ...
        * Doesn't create redirect
        ...
        assert response["X-Robots-Tag"] == "noindex"
        assert_no_cache_header(response)
        assert Document.objects.get(slug=d.slug, locale=d.locale).title == new_title
        assert not Document.objects.filter(title=old_title).exists()
    ...
    def test_slug_change_ignored_for_iframe(self):
        ...
        data = new_document_data()
        data.update({"title": rev.document.title, "slug": new_slug, "form": "rev"})
        response = self.client.post(
            "%s?iframe=1" % reverse("wiki.edit", args=[rev.document.slug]),
            data,
            HTTP_HOST=settings.WIKI_HOST,
        )
        assert response.status_code == 200
        ...
        assert (
            Document.objects.get(
                slug=rev.document.slug, locale=rev.document.locale
            ).slug
            == old_slug
        )
    ...
    def test_slug_collision_errors(self):
        ...
        data.update({"form-type": "rev", "slug": exist_slug})
        resp = self.client.post(
            reverse("wiki.edit", args=["some-new-title"]),
            data,
            HTTP_HOST=settings.WIKI_HOST,
        )
        assert resp.status_code == 200
    ...
    def test_redirect_can_be_clobbered(self):
        ...
        data.update({"form-type": "rev", "title": changed_title, "slug": changed_slug})
        resp = self.client.post(
            reverse("wiki.edit", args=[exist_slug]), data, HTTP_HOST=settings.WIKI_HOST
        )
        assert resp.status_code == 302
        assert resp["X-Robots-Tag"] == "noindex"
        assert_no_cache_header(resp)
        ...
        data.update({"form-type": "rev", "title": exist_title, "slug": exist_slug})
        resp = self.client.post(
            reverse("wiki.edit", args=[changed_slug]),
            data,
            HTTP_HOST=settings.WIKI_HOST,
        )
        assert resp.status_code == 302
    ...
    def test_slug_revamp(self):
        ...
        child_url = reverse("wiki.create") + "?parent=" + str(parent_doc.id)
        response = self.client.post(child_url, child_data, HTTP_HOST=settings.WIKI_HOST)
        assert response.status_code == 302
        # grab new revision ID
        child = Document.objects.get(locale="en-US", slug="length/length")
        rev_id = child.current_revision.id
        self.assertRedirects(response, reverse("wiki.document", args=["length/length"]))
        ...
        response = self.client.post(edit_url, child_data, HTTP_HOST=settings.WIKI_HOST)
        assert response.status_code == 302
        assert response["X-Robots-Tag"] == "noindex"
        assert_no_cache_header(response)
    ...
    def test_translate_keeps_toc_depth(self):
        ...
        en_doc = document(
            title="Eng Doc", slug=original_slug, is_localizable=True, locale=locale
        )
        en_doc.save()
    ...
    def test_translate_rebuilds_source_json(self):
        ...
        es_locale = "es"
        es_slug = "es-doc"
        en_doc = document(title="EN Doc", slug=en_slug, is_localizable=True)
        en_doc.save()
        en_doc.render()

        en_doc = Document.objects.get(
            locale=settings.WIKI_DEFAULT_LANGUAGE, slug=en_slug
        )
        ...
        es_translation_json = [
            t
            for t in new_en_json["translations"]
            if t["title"] == translation_data["title"]
        ][0]
        assert (
            es_translation_json["last_edit"]
    ...
    def test_slug_translate(self):
        ...
        en_doc = document(title="Eng Doc", slug=original_slug, is_localizable=True)
        en_doc.save()
        r = revision(document=en_doc)
        r.save()
        ...
        response = self.client.get(
            reverse("wiki.edit", args=[foreign_slug], locale=foreign_locale),
            HTTP_HOST=settings.WIKI_HOST,
        )
        page = pq(response.content)
    ...
    def test_restore_translation_source(self):
        ...
        # Create french doc
        data.update(
            {"locale": "fr", "title": "A Tést Articlé", "content": "C'ést bon."}
        )
        self.client.post(
            reverse("wiki.create", locale="fr"), data, HTTP_HOST=settings.WIKI_HOST
        )
    ...
    def test_tags_showing_correctly_after_doc_update(self):
        ...
        response = self.client.get(
            doc.get_absolute_url(), follow=True, HTTP_HOST=settings.WIKI_HOST
        )
        assert response.status_code == 200
    ...
    def test_review_tags(self, get_current):
        ...
        doc = Document.objects.get(
            locale=settings.WIKI_DEFAULT_LANGUAGE, slug="a-test-article"
        )
        rev = doc.revisions.order_by("-id").all()[0]
        ...
        response = self.client.get(
            reverse("wiki.list_review"), HTTP_HOST=settings.WIKI_HOST
        )
        assert response.status_code == 200
    ...
    def test_quick_review(self):
        ...
        for data_dict in test_data:
            ...
            rev = doc.revisions.order_by("-id").all()[0]
            review_url = reverse("wiki.quick_review", args=[doc.slug])

            params = dict(data_dict["params"], revision_id=rev.id)
            resp = self.client.post(review_url, params, HTTP_HOST=settings.WIKI_HOST)
            assert resp.status_code == 302
            assert_no_cache_header(resp)
    ...
    def test_edit_midair_collisions(self, is_ajax=False, translate_locale=None):
        ...
        # Edit #1 starts...
        resp = self.client.get(
            reverse("wiki.edit", args=[doc.slug]), HTTP_HOST=settings.WIKI_HOST
        )
        page = pq(resp.content)
    ...
    def test_multiple_edits_ajax(self, translate_locale=None):
        ...
        data.update({"form-type": "rev", "content": "Edit #1", "current_rev": rev_id1})
        ...
        # Edit #2 submits successfully
        data.update({"form-type": "rev", "content": "Edit #2", "current_rev": rev_id2})
    ...
    def test_toc_toggle_on(self):
        ...
        new_r.save()
        assert not Document.objects.get(
            slug=rev.document.slug, locale=rev.document.locale
        ).show_toc
        data = new_document_data()
        data["form-type"] = "rev"
    ...
    def test_repair_breadcrumbs(self):
        ...
        resp = self.client.get(
            reverse("wiki.repair_breadcrumbs", args=[french_bottom.slug], locale="fr"),
            HTTP_HOST=settings.WIKI_HOST,
        )
        assert resp.status_code == 302
        assert_no_cache_header(resp)
    ...
    def test_discard_location(self):
        ...
        assert pq(response.content).find(".btn-discard").attr("href") == reverse(
            "wiki.document", args=[doc.slug]
        )

        # Test that the 'discard button on a new translation goes
        # to the en-US page'
        response = self.client.get(
            reverse("wiki.translate", args=[doc.slug]),
            {"tolocale": "es"},
            HTTP_HOST=settings.WIKI_HOST,
        )
        assert pq(response.content).find(".btn-discard").attr("href") == reverse(
        ...
        response = self.client.get(
            reverse("wiki.edit", args=[foreign_doc.slug], locale=foreign_doc.locale),
            HTTP_HOST=settings.WIKI_HOST,
        )
        assert pq(response.content).find(".btn-discard").attr("href") == reverse(
            "wiki.document", args=[foreign_doc.slug], locale=foreign_doc.locale
        )

    @override_config(KUMASCRIPT_TIMEOUT=1.0)
    @mock.patch(
        "kuma.wiki.kumascript.get", return_value=("lorem ipsum dolor sit amet", None)
    )
    def test_revert(self, mock_kumascript_get):
        ...
        doc = Document.objects.get(
            locale=settings.WIKI_DEFAULT_LANGUAGE, slug="test-article-for-reverting"
        )
        rev = doc.revisions.order_by("-id").all()[0]
        ...
        response = self.client.post(
            reverse("wiki.revert_document", args=[doc.slug, rev.id]),
    ...
    def test_revert_moved(self):
        ...
        resp = self.client.post(
            reverse("wiki.revert_document", args=[doc.slug, prev_rev_id]),
            HTTP_HOST=settings.WIKI_HOST,
        )
    ...
    def test_email_for_first_edits(self):
        ...
        data = new_document_data()
        slug = "test-article-for-storing-revision-ip"
        data.update({"title": "A Test Article For First Edit Emails", "slug": slug})
        self.client.post(reverse("wiki.create"), data, HTTP_HOST=settings.WIKI_HOST)
        ...
        data.update(
            {
                "content": "Admin first edit should send an email",
    ...
    def test_email_for_watched_edits(self):
        ...
        assert (
            add_utm(
                get_compare_url(rev.document, rev.previous.id, rev.id), "Wiki Doc Edits"
            )
            in message.body
        )
    ...
    def test_email_for_child_edit_in_watched_tree(self):
        ...
        resp = self.client.post(
            reverse("wiki.edit", args=[child_doc.slug]),
            data,
            HTTP_HOST=settings.WIKI_HOST,
        )
        assert resp.status_code == 302
    ...
    def test_email_for_grandchild_edit_in_watched_tree(self):
        ...
        self.client.post(
            reverse("wiki.edit", args=[grandchild_doc.slug]),
            data,
            HTTP_HOST=settings.WIKI_HOST,
        )
        assert len(mail.outbox) == 1
...
class SectionEditingResourceTests(UserTestCase, WikiTestCase):
    ...
    def test_raw_with_editing_links_source(self):
        ...
        response = self.client.get(
            "%s?raw=true&edit_links=true"
            % reverse("wiki.document", args=[rev.document.slug]),
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
            HTTP_HOST=settings.WIKI_HOST,
        )
        assert response.status_code == 200
    ...
    def test_midair_section_merge_ajax(self):
        ...
        expected = """
            ...
            <p>test</p>
        """
        data = {"form-type": "rev", "content": rev.content, "slug": ""}

        # Edit #1 starts...
        resp = self.client.get(
            "%s?section=s1" % reverse("wiki.edit", args=[rev.document.slug]),
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
            HTTP_HOST=settings.WIKI_HOST,
        )
        assert resp.status_code == 200
        ...
        # Edit #2 starts...
        resp = self.client.get(
            "%s?section=s2" % reverse("wiki.edit", args=[rev.document.slug]),
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
            HTTP_HOST=settings.WIKI_HOST,
        )
        page = pq(resp.content)
    ...
    def test_midair_section_collision_ajax(self):
        ...
        replace_2 = """
            ...
            <p>first replace</p>
        """
        data = {"form-type": "rev", "content": rev.content}
        ...
        history_url = reverse(
            "wiki.document_revisions", kwargs={"document_path": rev.document.slug}
        )
        collision_err = MIDAIR_COLLISION % {"url": history_url}
    ...
    def test_raw_include_option(self):
        ...
        """
        resp = self.client.get(
            "%s?raw&include" % reverse("wiki.document", args=[rev.document.slug]),
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
            HTTP_HOST=settings.WIKI_HOST,
        )
        assert resp.status_code == 200
        assert_shared_cache_header(resp)
        assert normalize_html(expected) == normalize_html(
...
class MindTouchRedirectTests(UserTestCase, WikiTestCase):
    ...
    namespace_urls = (
        ...
        {"mindtouch": "/Help:Foo", "kuma": "%s/Help:Foo" % server_prefix},
        {"mindtouch": "/Help_talk:Foo", "kuma": "%s/Help_talk:Foo" % server_prefix},
        {
        ...
        {"mindtouch": "/Special:Foo", "kuma": "%s/Special:Foo" % server_prefix},
        {"mindtouch": "/Talk:en/Foo", "kuma": "%s/Talk:Foo" % server_prefix},
        {"mindtouch": "/Template:Foo", "kuma": "%s/Template:Foo" % server_prefix},
        {"mindtouch": "/User:Foo", "kuma": "%s/User:Foo" % server_prefix},
    )
...
class DeferredRenderingViewTests(UserTestCase, WikiTestCase):
    ...
    def test_rendered_content(self, mock_kumascript_get):
        ...
        resp = self.client.get(self.url, follow=False, HTTP_HOST=settings.WIKI_HOST)
        p = pq(resp.content)
    ...
    @mock.patch("kuma.wiki.kumascript.get")
    def test_schedule_rendering(
        self, mock_kumascript_get, mock_document_schedule_rendering
    ):
        mock_kumascript_get.return_value = (self.rendered_content, None)
...
class PageMoveTests(UserTestCase, WikiTestCase):
    def test_move_conflict(self):
        ...
        child = revision(
            title="Child of page-move view test",
        ...
        revision(
            title="Conflict for page-move view",
