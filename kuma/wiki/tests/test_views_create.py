...
def test_check_read_only_mode(user_client):
    response = user_client.get(reverse("wiki.create"), HTTP_HOST=settings.WIKI_HOST)
    assert response.status_code == 403
    assert_no_cache_header(response)


def test_user_add_document_permission(editor_client):
    response = editor_client.get(reverse("wiki.create"), HTTP_HOST=settings.WIKI_HOST)
    assert response.status_code == 403
...
def test_create_valid(add_doc_client):
    ...
    assert sorted(doc.tags.all().values_list("name", flat=True)) == ["tag1", "tag2"]
...
def test_create_child_valid(root_doc, add_doc_client, slug):
    ...
    assert resp["X-Robots-Tag"] == "noindex"
    assert_no_cache_header(resp)
    assert resp["Location"].endswith(reverse("wiki.document", args=(full_slug,)))
...
def test_clone_get(root_doc, add_doc_client):
    ...
    assert (
        page.find("textarea[name=content]")[0].value.strip()
