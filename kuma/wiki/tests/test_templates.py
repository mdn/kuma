...
def test_deletion_log_assert(db, rf):
    ...
    assert str(exc.value) == (
        "Failed assertion: Deletion log details are only" " for moderators."
    )
...
class DocumentTests(UserTestCase, WikiTestCase):
    ...
    def test_document_view(self):
        ...
        response = self.client.get(
            r.document.get_absolute_url(), HTTP_HOST=settings.WIKI_HOST
        )
    ...
    def test_document_no_breadcrumbs(self):
        ...
        response = self.client.get(d1.get_absolute_url(), HTTP_HOST=settings.WIKI_HOST)
        assert 200 == response.status_code
        doc = pq(response.content)
        assert doc("main#content div.document-head h1").text() == d1.title
        assert len(doc("nav.crumbs")) == 0

        response = self.client.get(d2.get_absolute_url(), HTTP_HOST=settings.WIKI_HOST)
        assert 200 == response.status_code
        doc = pq(response.content)
    ...
    def test_translation_document_no_approved_content(self):
        ...
        assert (
            "This article doesn't have approved content yet."
            in doc("article#wikiArticle").text()
        ) or (
            "Cet article n'a pas encore de contenu" in doc("article#wikiArticle").text()
        )
    ...
    def test_redirect(self):
        ...
        response = self.client.get(
            redirect_url, follow=True, HTTP_HOST=settings.WIKI_HOST
        )
    ...
    def test_redirect_from_nonexistent(self):
        ...
        response = self.client.get(
            urlparams(d.get_absolute_url()), HTTP_HOST=settings.WIKI_HOST
        )
    ...
    def test_non_localizable_translate_disabled(self):
        ...
        resp = self.client.get(d.get_absolute_url(), HTTP_HOST=settings.WIKI_HOST)
        doc = pq(resp.content)
    ...
    def test_toc_depth(self):
        ...
        new_r = revision(
            document=r.document, content=r.content, toc_depth=0, is_approved=True
        )
        new_r.save()
        response = self.client.get(
            r.document.get_absolute_url(), HTTP_HOST=settings.WIKI_HOST
...
@override_settings(
    CONTENT_EXPERIMENTS=_TEST_CONTENT_EXPERIMENTS,
    PIPELINE=_PIPELINE,
...
class DocumentContentExperimentTests(UserTestCase, WikiTestCase):
    ...
    script_src = 'src="%sbuild/js/experiment-framework-test.js"' % settings.STATIC_URL
    ...
    def test_anon_no_variant_selected(self):
        ...
        rev = revision(
            save=True, content="Original Content.", is_approved=True, slug="Original"
        )
        response = self.client.get(
    ...
    def test_anon_valid_variant_selected(self):
        ...
        response = self.client.get(
            rev.document.get_absolute_url(), {"v": "test"}, HTTP_HOST=settings.WIKI_HOST
        )
        assert response.status_code == 200
...
class GoogleAnalyticsTests(UserTestCase, WikiTestCase):
    ...
    def test_en_doc(self):
        ...
        assert doc.slug
        response = self.client.get(doc.get_absolute_url(), HTTP_HOST=settings.WIKI_HOST)
        assert response.status_code == 200
        content = response.content.decode(response.charset)
    ...
    def test_fr_doc(self):
        ...
        fr_doc = _create_document(title="Document Français", parent=en_doc, locale="fr")
        assert en_doc.slug != fr_doc.slug
        response = self.client.get(
            fr_doc.get_absolute_url(), HTTP_HOST=settings.WIKI_HOST
        )
        assert response.status_code == 200
        content = response.content.decode(response.charset)
        assert self.ga_create in content
    ...
    def test_orphan_doc(self):
        ...
        response = self.client.get(
            orphan_doc.get_absolute_url(), HTTP_HOST=settings.WIKI_HOST
        )
        assert response.status_code == 200
        content = response.content.decode(response.charset)
        assert self.ga_create in content
...
class NewDocumentTests(UserTestCase, WikiTestCase):
    ...
    def test_new_document_GET_with_perm(self):
        ...
        self.client.login(username="admin", password="testpass")
        response = self.client.get(reverse("wiki.create"), HTTP_HOST=settings.WIKI_HOST)
        assert 200 == response.status_code
        doc = pq(response.content)
    ...
    def test_new_document_POST(self, get_current):
        ...
        response = self.client.post(
            reverse("wiki.create"), data, follow=True, HTTP_HOST=settings.WIKI_HOST
        )
    ...
    def test_new_document_other_locale(self, get_current):
        ...
        locale = "es"
        self.client.post(
            reverse("wiki.create", locale=locale),
            data,
            follow=True,
    ...
    def test_slug_collision_validation(self):
        ...
        ul = doc("article ul.errorlist")
        assert 1 == len(ul)
        assert "Document with this Slug and Locale already exists." == ul("li").text()
...
class NewRevisionTests(UserTestCase, WikiTestCase):
    ...
    def test_new_revision_POST_document_with_current(self):
        ...
        response = self.client.post(edit_url, data, HTTP_HOST=settings.WIKI_HOST)
        assert response.status_code == 302
        assert response["X-Robots-Tag"] == "noindex"
        assert_no_cache_header(response)
    ...
    def test_new_revision_POST_document_without_current(self, get_current, edited_fire):
        ...
        data["form-type"] = "rev"
        response = self.client.post(
            reverse("wiki.edit", args=[self.d.slug]), data, HTTP_HOST=settings.WIKI_HOST
        )
        assert response.status_code == 302
        assert response["X-Robots-Tag"] == "noindex"
        assert_no_cache_header(response)
...
class TranslateTests(UserTestCase, WikiTestCase):
    ...
    def test_translate_GET_with_perm(self):
        ...
        response = self.client.get(self._translate_uri(), HTTP_HOST=settings.WIKI_HOST)
        assert response.status_code == 200
        assert response["X-Robots-Tag"] == "noindex"
        assert_no_cache_header(response)
        doc = pq(response.content)
    ...
    def test_invalid_document_form(self):
        ...
        response = self.client.post(translate_uri, data, HTTP_HOST=settings.WIKI_HOST)
        assert response.status_code == 200
        assert response["X-Robots-Tag"] == "noindex"
        assert_no_cache_header(response)
    ...
    def test_another_translation_to_locale(self, get_current, edited_fire):
        ...
        assert doc("article.approved .translate-rendered").text() == rev_enUS.content
    ...
    def test_translate_update_rev_and_doc(self):
        ...
        assert response["X-Robots-Tag"] == "noindex"
        assert_no_cache_header(response)
        assert response["location"].endswith("/es/docs/un-test-articulo?rev_saved=")
        revisions = rev_es.document.revisions.all()
...
def _test_form_maintains_based_on_rev(
    client, doc, view, post_data, trans_lang=None, locale=None
):
    ...
    if trans_lang:
        ...
        uri = quote(reverse("wiki.translate", locale=trans_lang, args=[translate_path]))
    else:
    ...
    assert (
        int(pq(response.content)("input[name=based_on]").attr("value")) == orig_rev.id
    )
...
class ArticlePreviewTests(UserTestCase, WikiTestCase):
    ...
    def test_preview_GET_405(self):
        ...
        response = self.client.get(
            reverse("wiki.preview"), HTTP_HOST=settings.WIKI_HOST
        )
...
class SelectLocaleTests(UserTestCase, WikiTestCase):
    ...
    def test_page_renders_locales(self):
        ...
        response = self.client.get(
            reverse("wiki.select_locale", args=[self.d.slug]),
            HTTP_HOST=settings.WIKI_HOST,
        )
        assert response.status_code == 200
        assert_no_cache_header(response)
        doc = pq(response.content)
        assert (
            len(doc("#select-locale ul.locales li")) == len(settings.LANGUAGES) - 1
        )  # All except for 1 (en-US)


def _create_document(
    title="Test Document", parent=None, locale=settings.WIKI_DEFAULT_LANGUAGE
):
    d = document(
        title=title,
...
@pytest.mark.parametrize(
    "elem_num,has_prev,is_english,has_revert",
    [(0, True, False, False), (1, True, False, True), (2, False, True, False)],
    ids=["current", "first_trans", "en_source"],
)
def test_list_revisions(
    elem_num, has_prev, is_english, has_revert, admin_client, trans_edit_revision
):
    ...
    url = reverse("wiki.document_revisions", locale=doc.locale, args=[doc.slug])
    ...
    revision_url = reverse(
        "wiki.revision", locale=rev_doc.locale, args=[rev_doc.slug, revision.id]
    )
