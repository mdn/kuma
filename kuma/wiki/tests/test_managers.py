...
def redirect_doc(root_doc, wiki_user):
    """A redirect document."""
    html = REDIRECT_CONTENT % {
        "href": root_doc.get_absolute_url(),
...
def archive_doc(wiki_user):
    ...
    archive = Document.objects.create(
        locale="en-US", slug="Archive", title="Archive of obsolete content"
    )
...
def test_get_queryset(root_doc):
    ...
    deleted_doc = Document.objects.create(
        locale="en-US", slug="Deleted", title="Deleted Document", deleted=True
    )
...
def test_get_natural_key(root_doc):
    ...
    assert root_doc == Document.objects.get_by_natural_key(
...
def test_documents_filter_for_list_by_noparent(root_doc, trans_doc):
    ...
    result = Document.objects.filter_for_list(locale=trans_doc.locale, noparent=True)
    assert len(result) == 0
...
def test_documents_filter_with_localization_tag_by_locale(
    create_revision, trans_revision
):
...
def test_documents_filter_with_localization_tag_excludes_redirects(redirect_doc):
