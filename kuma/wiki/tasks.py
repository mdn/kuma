...
from .models import (Document, DocumentDeletionLog,
                     ...
                     Revision, RevisionIP)
from .rerender import SafeRenderJob
...
def run_safe_render_job(job_id):
    """Run the next steps in a safe re-render job."""
    job = SafeRenderJob.load(job_id)
    call_again, with_timeout = job.run()
    if call_again:
        run_safe_render_job.apply_async(args=(job_id,),
                                        countdown=with_timeout)
