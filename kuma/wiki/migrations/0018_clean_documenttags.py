...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        At some point we created some tags with identical names.
        ...
        for tag in tags:
            if tag.name in tag_names:
                dupe_tag_names.append(tag.name)
            tag_names.append(tag.name)
        # loop thru the dupes
        for dupe_name in dupe_tag_names:
            doc_tags = orm.DocumentTag.objects.filter(name=dupe_name)
            # keep the first
            canonical_doc_tag = doc_tags[0]
            # re-assign the other tagged items to the canonical tag
            for dupe_tag in doc_tags[1:]:
                tagged_items = TaggedItem.objects.filter(tag=dupe_tag)
                tagged_items.update(tag=canonical_doc_tag)
                # delete the dupe tag
                dupe_tag.delete()
