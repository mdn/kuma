    ...
    function displayRestoreDraft(startingContent) {
        ...
        if (prevDraft && !draftMatchesStart) {
            ...
            if (draftType === 'old') {
                ...
                draftWarning += ' <a href="' + historyUrl + '" target="_blank" rel="noopener" class="external-icon">' + gettext('Revision history.') + '</a> ';
                draftWarning += '<a href="' + publishUrl + '" target="_blank" rel="noopener" class="external-icon">' + gettext('Published version') + '</a>';
                draftWarning += '</div>';
