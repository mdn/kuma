...
def test_verify_valid_key(spam_check_everyone, constance_config, mock_requests):
...
def test_verify_invalid_key(spam_check_everyone, constance_config, mock_requests):
    constance_config.AKISMET_KEY = "api-key"
...
def test_verify_invalid_key_wrong_response(
    spam_check_everyone, constance_config, mock_requests
):
    constance_config.AKISMET_KEY = "api-key"
...
def test_exception_recording(
    mock_record_exception, spam_check_everyone, constance_config, mock_requests
):
...
def test_check_comment_ham(spam_check_everyone, constance_config, mock_requests):
    constance_config.AKISMET_KEY = "comment"
    mock_requests.post(VERIFY_URL, content=b"valid")
    client = Akismet()
    assert client.ready
    ...
    valid = client.check_comment("0.0.0.0", "Mozilla", comment_content="yada yada")
    assert valid
...
def test_check_comment_spam(spam_check_everyone, constance_config, mock_requests):
    constance_config.AKISMET_KEY = "comment"
    mock_requests.post(VERIFY_URL, content=b"valid")
...
def test_check_comment_wrong_response(
    spam_check_everyone, constance_config, mock_requests
):
    constance_config.AKISMET_KEY = "comment"
    mock_requests.post(VERIFY_URL, content=b"valid")
    ...
    with pytest.raises(AkismetError) as excinfo:
        client.check_comment("0.0.0.0", "Mozilla", comment_content="yada yada")
    exc = excinfo.value
...
def test_submit_spam_success(spam_check_everyone, constance_config, mock_requests):
    constance_config.AKISMET_KEY = "spam"
    mock_requests.post(VERIFY_URL, content=b"valid")
    client = Akismet()
    mock_requests.post(SPAM_URL, content=client.submission_success.encode())
    result = client.submit_spam(
        "0.0.0.0", "Mozilla", comment_content="spam. spam spam. spam."
    )
    assert result is None


def test_submit_spam_failure(spam_check_everyone, constance_config, mock_requests):
    constance_config.AKISMET_KEY = "spam"
    mock_requests.post(VERIFY_URL, content=b"valid")
    client = Akismet()
    mock_requests.post(SPAM_URL, content=b"something completely different")
    with pytest.raises(AkismetError) as excinfo:
        client.submit_spam("0.0.0.0", "Mozilla", comment_content="spam. eggs.")
    exc = excinfo.value
...
def test_submit_ham_success(spam_check_everyone, constance_config, mock_requests):
    constance_config.AKISMET_KEY = "spam"
    mock_requests.post(VERIFY_URL, content=b"valid")
    client = Akismet()
    mock_requests.post(HAM_URL, content=client.submission_success.encode())
    result = client.submit_ham(
        "0.0.0.0", "Mozilla", comment_content="ham and bacon and pork."
    )
    assert result is None


def test_submit_ham_failure(spam_check_everyone, constance_config, mock_requests):
    constance_config.AKISMET_KEY = "spam"
    mock_requests.post(VERIFY_URL, content=b"valid")
    client = Akismet()
    ...
    with pytest.raises(AkismetError) as excinfo:
        client.submit_ham(
            "0.0.0.0", "Mozilla", comment_content="eggs with ham. ham with eggs."
        )
    exc = excinfo.value
    assert exc.status_code == 200
    assert exc.debug_help == "Not provided"
