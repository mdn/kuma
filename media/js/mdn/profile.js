...
(function () {
    ...
    var updateTaglistFromField = debounce(function () {
        var taglist = $('#tags-expertise'),
            ...
            e_tags = expertise.val().split(',');
        $('#tags-expertise .tag-expert input[type=checkbox]')
            .removeAttr('checked');
        $.each(e_tags, function (idx, tag) {
            tag = $.trim(tag);
            $('#tags-expertise .tag-expert input[value="'+tag+'"]')
