...
class kuma_config {
    ...
    exec {
        ...
        "kuma_south_migrate":
            user => "vagrant",
            cwd => "/home/vagrant/src",
            command => "/home/vagrant/env/bin/python manage.py migrate events --fake",
            require => [ Exec["kuma_django_syncdb"] ];
        "kuma_south_migrate2":
            user => "vagrant",
            cwd => "/home/vagrant/src",
            command => "/home/vagrant/env/bin/python manage.py migrate users 0002 --fake",
            require => [ Exec["kuma_south_migrate"] ];
        "kuma_south_migrate3":
            user => "vagrant",
            cwd => "/home/vagrant/src",
            command => "/home/vagrant/env/bin/python manage.py migrate --delete-ghost-migrations --noinput",
